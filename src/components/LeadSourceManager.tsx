  ...
  return <Card className="p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="secondary" className="bg-zinc-950">{leadSources.length}</Badge>
        </div>
      ...
      {showAddForm && <div className="mb-4 p-4 border rounded-lg bg-muted/50">
          ...
          <div className="space-y-3">
            <Input placeholder="Nombre de la fuente (ej. Website, Referencia, LinkedIn)" value={newSourceName} onChange={e => setNewSourceName(e.target.value)} />
      ...
      <div className="space-y-2">
        {leadSources.length === 0 ? <div className="text-center py-8 text-zinc-700 dark:text-zinc-300">
            No hay fuentes de lead registradas
          </div> : leadSources.map(source => <div key={source.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
        ...
        <div className="flex-1 flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <Input placeholder="Nombre de la fuente" value={editName} onChange={e => setEditName(e.target.value)} className="h-8" />
                    <p className="text-xs text-muted-foreground mt-1">
                      Creada: {new Date(source.created_at).toLocaleDateString()}
                    </p>
                  </div>
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="sm" onClick={handleEditSave} disabled={isUpdating || !editName.trim()}>
        ...
        <>
                  ...
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="sm" onClick={() => handleEditStart(source)} disabled={editingSource !== null || isUpdating}>
                      <Edit className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleDeleteSource(source.id, source.name)} disabled={deletingId === source.id || isDeleting || editingSource !== null}>

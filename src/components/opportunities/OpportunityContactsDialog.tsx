...
export const OpportunityContactsDialog: React.FC<OpportunityContactsDialogProps> = ({
  ...
  return <TooltipProvider>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto bg-slate-50">
          <DialogHeader>
          ...
          <div className="space-y-6">
                  ...
                  <form onSubmit={handleSubmitForm} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input id="name" value={formData.name} onChange={e => handleInputChange('name', e.target.value)} placeholder="Nombre completo" required />
                      </div>
                      
                      <div className="space-y-2">
                        <Label htmlFor="position">Cargo</Label>
                        <Input id="position" value={formData.position} onChange={e => handleInputChange('position', e.target.value)} placeholder="Cargo o posición" />
                      </div>
                      
                      <div className="space-y-2">
                        <Label htmlFor="email">Email</Label>
                        <Input id="email" type="email" value={formData.email} onChange={e => handleInputChange('email', e.target.value)} placeholder="email@ejemplo.com" />
                      </div>
                      
                      <div className="space-y-2">
                        <Label htmlFor="phone">Teléfono</Label>
                        <Input id="phone" value={formData.phone} onChange={e => handleInputChange('phone', e.target.value)} placeholder="+1234567890" />
                      </div>
                      
                      <div className="space-y-2 md:col-span-2">
                        <Label htmlFor="linkedin">LinkedIn</Label>
                        <Input id="linkedin" value={formData.linkedin_url} onChange={e => handleInputChange('linkedin_url', e.target.value)} placeholder="https://linkedin.com/in/usuario" />
                      </div>
                    </div>
            ...
            {/* Add Contact Button */}
            {!showAddForm && <Button onClick={handleAddContact} className="w-full">
            ...
            <div className="space-y-4">
              {contacts.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                ...
                </div> : contacts.map(contact => <Card key={contact.id} className="hover:shadow-md transition-shadow">
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start">
                        <div className="flex-1 space-y-2">
                          <div className="flex items-center gap-2">
                            <h3 className="font-semibold text-lg">{contact.name}</h3>
                            {contact.position && <Badge variant="secondary" className="flex items-center gap-1">
                          ...
                          <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                            ...
                            {contact.phone && <div className="flex items-center gap-1">
                            ...
                            {contact.linkedin_url && <div className="flex items-center gap-1">
                                <Linkedin className="w-4 h-4" />
                                <a href={contact.linkedin_url} target="_blank" rel="noopener noreferrer" className="hover:text-primary">
                                  LinkedIn
                                </a>
                              </div>}
                          </div>
                          ...
                          <div className="text-xs text-muted-foreground">
                            Creado: {format(new Date(contact.created_at), 'dd/MM/yyyy HH:mm', {
                        locale: es
                      })}
                          </div>
                        </div>
